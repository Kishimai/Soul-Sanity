...
public class Slime : MonoBehaviour
{
        ...
        if(Patrol == true){
            ...
            if(heightInfo.collider == null){
                ...
                if(movingRight == true){
                    ...
                    if(frontInfo.transform.gameObject.tag == "Design"){
                        if(movingRight == true){
                        transform.eulerAngles = new Vector3(0, 0, 0);
                        movingRight = false;
                    } else{
    ...
    private void OnCollisionEnter2D(Collision2D other) {
        if(other.gameObject.tag == "Player"){
            ...
            if(other.gameObject.GetComponent<PlayerController>().playerChecker.transform.position.y > this.transform.position.y){
